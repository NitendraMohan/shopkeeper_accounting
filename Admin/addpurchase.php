...
if(isset($_GET['id']) && $_GET['id']>0){
  $id=$_GET['id'];
  $row=mysqli_fetch_assoc($con->query("select * from purchase_order where id='$id'"));
  $order_number=$row['order_number'];
  $supplier_id=$row['sup_id'];
  $purchase_date=strtotime($row['purchase_date']);
  $order_date=date('d-m-Y',$purchase_date);
  // $dish=$row['dish'];
...
if(isset($_POST['submit'])){
  ...
  if($id!=''){
    $con->query("update purchase_order set category_id='$category_id',dish='$dish',dish_detail='$dish_detail' $image_status where id='$id'");
    redirect('purchase_order.php'); 
  }
  else{
    if(mysqli_num_rows(mysqli_query($con,"select * from purchase_order where order_number='$order_number'"))>0){
  ...
  $con->query("insert into purchase_order(sup_id, order_number, purchase_date, total_bill, status, added_on) values('$supplier_id','$order_number','$order_date','$total_bill',1,'$added_on')");
  ...
  foreach($dishArr as $key=>$val){
    ...
    $qty=$qtyArr[$key];
    $amount=$price*$qty;
    $con->query("insert into purchase_order_detail(purchase_order_id, product_id, product_qty, rate, price, status, added_on) values('$lastid','$dish','$qty','$price', '$amount',1 ,'$added_on')");
...
$res_supplier=$con->query("select * from supplier where status='1'");
?>
<div class="row">
  <h1 class="card-title ml10">Manage Purchase Order</h1>
          ...
          <div class="card">
            <div class="card-body">
              <form class="forms-sample" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="exampleInputEmail3">Supplier Name</label>
                  <select class="form-control" name="supplier_id" id="supplier" placeholder="Supplier Name">
                      <option value="">Select Supplier</option>
                      <?php 
                        while ($s_row=$res_supplier->fetch_assoc()) {
                          if($s_row['id']==$supplier_id){
