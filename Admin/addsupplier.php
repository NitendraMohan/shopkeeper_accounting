...
if(isset($_GET['id']) && $_GET['id']>0){
  $id=$_GET['id'];
  $row=mysqli_fetch_assoc($con->query("select * from supplier where id='$id'"));
  $name=$row['name'];
  $mobile=$row['mobile'];
  $address=$row['address'];
...
if(isset($_POST['submit'])){
  ...
  if($id!=''){
    $con->query("update supplier set name='$name', mobile='$mobile',address='$address' where id=$id");
    redirect('supplier.php');  
  }
  else{
    if(mysqli_num_rows(mysqli_query($con,"select * from supplier where mobile='$mobile'"))>0){
    $msg="supplier already added";
  }
  else{
  $con->query("insert into supplier(name,mobile,address,status,added_on) values('$name'
...
?>
<div class="row">
  <h1 class="card-title ml10">Manage Supplier</h1>
